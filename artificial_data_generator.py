...
class Main():
    def __init__(self, flag, z_num):
        self.num_arm = 8
        self.N = 16
        self.feature_dim = self.num_arm * self.N
        self.aleph_opt = z_num
    ...
    def one_hot_data_sampling(self):
        ...
        ##ランクに応じた報酬確率定義
        max_p = self.aleph_opt
        diff_p = max_p / self.num_arm #腕の差分計算(等間隔)
        p_seed = np.arange(0+diff_p, (self.aleph_opt-0.05)+diff_p, diff_p)#希求水準にあった報酬確率生成
        #print("p_seed:", p_seed)
...
args = sys.argv
FLAG = args[1] #one-hot特徴量の場合 False, 混合特徴量の場合 True
z_num = args[2]
main = Main(FLAG, z_num)
if FLAG == True:
